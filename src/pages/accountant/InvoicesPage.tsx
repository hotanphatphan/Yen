...
// ─── Post Invoice Dialog (Step 4) ─────────────────────────────────────────────

function PostInvoiceDialog({
  invoice,
  onClose,
  onPosted,
}: {
  ...
  async function handlePost() {
    ...
    try {
      ...
      const { error: entryErr } = await supabase.from('journal_entries').insert(entry)
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl border border-slate-200 divide-y divide-slate-100 text-sm overflow-hidden">
            <div className="flex items-center gap-3 px-4 py-2.5 bg-slate-50">
              <span className="w-8 text-xs font-bold text-slate-400">Nợ</span>
              <select
                value={debitAccount}
              ...
              <span className="font-mono text-slate-600 text-xs">{fmt(invoice.subtotal)}</span>
            </div>
            <div className="flex items-center gap-3 px-4 py-2.5">
              <span className="w-8 text-xs font-bold text-slate-400">Có</span>
              <select
                value={creditAccount}
            ...
            <>
              ...
              <div className="rounded-xl border border-slate-200 divide-y divide-slate-100 text-sm overflow-hidden">
                <div className="flex items-center gap-3 px-4 py-2.5 bg-slate-50">
                  ...
                  <select
                    value={vatDebit}
                    onChange={e => setVatDebit(e.target.value)}
                    className="flex-1 bg-transparent text-slate-700 focus:outline-none text-sm"
                  >
                ...
                <div className="flex items-center gap-3 px-4 py-2.5">
                  ...
                  <select
                    value={vatCredit}
                    onChange={e => setVatCredit(e.target.value)}
                    className="flex-1 bg-transparent text-slate-700 focus:outline-none text-sm"
                  >
                    <option value="">— Không có —</option>
                    {COMMON_ACCOUNTS.map(a => (
...
export default function InvoicesPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (parsed: ParsedInvoice & { companyId: string }) => {
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['invoices', selectedCompanyId] }),
  })

  const processFiles = useCallback(async (files: File[]) => {
    if (!company) return
    const newItems: UploadItem[] = files.map(f => ({
      id: crypto.randomUUID(), file: f, state: 'pending',
    }))
    setUploads(prev => [...newItems, ...prev])
    for (const item of newItems) {
      setUploads(prev => prev.map(u => u.id === item.id ? { ...u, state: 'parsing' } : u))
      try {
        const result = await parseInvoiceFile(item.file, company.mst)
        await saveMutation.mutateAsync({ ...result, companyId: company.id })
  ...
  return (
    <AccountantLayout>
      <PageHeader
        title="Hóa đơn"
        subtitle="Import, phân tích và hạch toán hóa đơn"
      />
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          <label className="text-sm font-medium text-slate-500 shrink-0">Công ty:</label>
          <select
          ...
          {company && <span className="text-xs text-slate-400">MST: {company.mst}</span>}
        </div>
              ...
              <div className="space-y-2">
                {uploads.map(item => (
                  <div key={item.id}
                    ...
                    {item.state === 'parsing' && (
                      <span className="text-xs text-indigo-500 animate-pulse">Đang phân tích...</span>
                    )}
              ...
              <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
                ...
                <div className="flex gap-3 text-xs text-slate-500">
                  <span className="flex items-center gap-1">
                    <ArrowDownCircle className="h-3.5 w-3.5 text-orange-400" /> Mua vào
                  </span>
                  <span className="flex items-center gap-1">
                    <ArrowUpCircle className="h-3.5 w-3.5 text-green-500" /> Bán ra
                  </span>
                </div>
              </div>
                              ...
                              <tbody>
                                {inv.line_items.map((item, i) => {
                                  const li = item as {
                                    ...
                                    unitPrice: number; amount: number; vatRate: string
                                  }
                                  return (
